...
import com.example.talenta.data.model.Users
...
import com.example.talenta.presentation.ui.screens.sponsor.use_cases.SearchUserUseCase
...
data class UsersState(
    val isLoading: Boolean = false, val user: List<Users> = listOf(Users()), val error: String = ""
)
...
class SponsorViewModel @Inject constructor(
    ...
    private val _searchUser = MutableStateFlow(UserState())
    val searchedUser: StateFlow<UserState> = _searchUser
    ...
    fun search(field: String, query: String) {
        searchUserUseCase(field, query).onEach { result ->
            when (result) {
                is FirestoreResult.Loading -> {
                    _searchUser.value = UserState(isLoading = true)
                }

                is FirestoreResult.Success -> {
                    _searchUser.value = UserState(user = result.data)
